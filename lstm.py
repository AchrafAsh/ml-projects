...
class Generator():
	...
	def generate(self, init_str='I', predict_len=100, temperature=0.85):
		...
		for p in range(predict_len):
			output, (hidden, cell) = self.rnn(
										initial_input[p].view(1).to(device), hidden, cell)
			ouput_dit = output.data.view(-1).div(temperature).exp()
	...
	def train(self):
		...
		writer = SummaryWriter(f'runs/names') # for tensorboard
		...
		for epoch in range(1, self.num_epochs + 1):
			...
			self.rnn.zero_grad()
			loss = 0
			inp,  = inp.to(device), target.to(device)
